...
public class Circle {
	...
	private int createdById; // 🔥 추가 (모임 생성자의 ID)
	
	 // ✅ 개설자 정보 추가
    private String creatorName; // 개설자 이름
    private String creatorEmail; // 개설자 이메일
	...
	public Timestamp getCircleDateAsTimestamp() {
		...
		try {
			...
			return Timestamp
					.valueOf(LocalDateTime.parse(formattedDate, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
		} catch (Exception e) {
